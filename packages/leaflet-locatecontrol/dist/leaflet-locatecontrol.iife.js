(function () {
  ...
  */
  (function (factory, window) {
    ...
    var LDomUtilApplyClassesMethod = function LDomUtilApplyClassesMethod(
      method,
      element,
      classNames
    ) {
    ...
    var CompassMarker = LocationMarker.extend({
      ...
      _getIconSVG: function _getIconSVG(options, style) {
        ...
        var path = 'M0,0 l'
          .concat(options.width / 2, ',')
          .concat(options.depth, ' l-')
          .concat(w, ',0 z');
        var svgstyle = 'transform: rotate('.concat(this._heading, 'deg)');
        var svg =
          '<svg xmlns="http://www.w3.org/2000/svg" width="'
            .concat(w, '" height="')
            .concat(h, '" version="1.1" viewBox="-')
            .concat(w / 2, ' 0 ')
            .concat(w, ' ')
            .concat(h, '" style="')
            .concat(svgstyle, '">') +
    ...
    var LocateControl = L.Control.extend({
      options: {
         ...
         */
        createButtonCallback: function createButtonCallback(
          container,
          options
        ) {
        ...
        /** This event is called in case of any location error that is not a time out error. */ onLocationError:
          function onLocationError(err, control) {
            alert(err.message);
          },
        /**
         ...
         */
        onLocationOutsideMapBounds: function onLocationOutsideMapBounds(
          control
        ) {
          control.stop();
      ...
      onAdd: function onAdd(map) {
        ...
        var linkAndIcon = this.options.createButtonCallback(
          container,
          this.options
        );
